        ...
        <table class="table table-sm table-striped m-b-0">
            ...
            <tbody class="table-striped items sortableOptions">
                ...
                <tr class="array-row clonable" style="display: none;">
                    @foreach( $field['columns'] as $column)
                    <td>
                        <input class="form-control form-control-sm" type="{{ $column['type'] }}" data-cell-name="item.{{ $column['name'] }}" @if($column['type'] == 'checkbox') value="0" @endif>
                    </td>
                    @endforeach
                    <td>
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            function bpFieldInitTableElement(element) {
                ...
                if($rows != '[]') {
                    $.each($rows, function(key) {
                        ...
                        $.each(this, function(column , value) {
                            ...
                            if($tableWrapper.find('tbody tr:last').find('input[data-cell-name="item.' + column + '"]').attr('type') == 'checkbox'){
                                if(value == 1){
                                    $tableWrapper.find('tbody tr:last').find('input[data-cell-name="item.' + column + '"]').prop("checked", true);
                                }
                                if(value == 0){
                                    $tableWrapper.find('tbody tr:last').find('input[data-cell-name="item.' + column + '"]').prop("checked", false);
                                }
                                /*if(value == 'on'){ // Fix. Remove after all quizzes will be updated
                                    $tableWrapper.find('tbody tr:last').find('input[data-cell-name="item.' + column + '"]').prop("checked", false).val(0);
                                }*/
                                $tableWrapper.find('tbody tr:last').find('input[data-cell-name="item.' + column + '"]').change(function(){
                ...
                function addItem() {
                    $tableWrapper.find('tbody').append($tableWrapper.find('tbody .clonable').clone().show().removeClass('clonable')).find('input[type="checkbox"]').change(function(){
                                    if($(this).prop('checked')){
                                        $(this).val(1);
                                    } else {
                                        $(this).val(0);
